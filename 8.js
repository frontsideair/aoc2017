const instructions = `sdj dec 558 if r <= 8
...
function operatorToFunction(operator) {
  return {
    inc: (a, b) => a + b,
    dec: (a, b) => a - b,
    "==": (a, b) => a == b,
...
function parse(instruction) {
  [reg, op1, amt, _, lhs, op2, rhs] = instruction.split(" ");

  return { reg, op1, amt: Number(amt), op2, lhs, rhs };
}
...
instructions.forEach(instruction => {
  const { reg, op1, amt, op2, lhs, rhs } = parse(instruction);
  const lhsVal = registers.get(lhs) || 0;
  const regVal = registers.get(reg) || 0;

  if (operatorToFunction(op2)(lhsVal, rhs)) {
    const val = operatorToFunction(op1)(regVal, amt);
    max = Math.max(max, val);
    registers.set(reg, val);
  }
});

console.log(Math.max(...registers.values()), max);
